...
def initialising():
    con = connection()
    con.execute("""create or replace database USER_DB_SWAN;   """)
    con.execute("create or replace schema raw ;")

    con.execute("""CREATE TABLE IF NOT EXISTS USER_DB_SWAN.raw.user_session_channel (
        userId int not NULL,
        sessionId varchar(32) primary key,
        channel varchar(32) default 'direct'  
    );""" )

    con.execute("""CREATE TABLE IF NOT EXISTS USER_DB_SWAN.raw.session_timestamp (
        sessionId varchar(32) primary key,
        ts timestamp )""")
...
def transfer():    
    ...
    FROM @USER_DB_SWAN.raw.blob_stage/user_session_channel.csv;
    """)
